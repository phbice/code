...
VOID CreateD2DResource(HWND hWnd)
{
    if (!g_pRenderTarget)
    {
        ...
        if (FAILED(hr))
        {
            MessageBox(hWnd, TEXT("Create brush failed!"), TEXT("Error"), 0) ;
            return ;
        }
    }
}
...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine, int iCmdShow )
{
    ...
    winClass.lpszClassName = TEXT("Direct2D");
    ...
    HWND hwnd = CreateWindowEx(NULL,  
        TEXT("Direct2D"),           // window class name
        TEXT("Draw Rectangle"),		// window caption
